...
function processIngredient(ingredient: Ingredient) {
	switch (ingredient.type) {
		case "flour":
			return handleFlour(ingredient);
		case "water":
			return handleWater(ingredient);
		default:
			return assertUnreachable(ingredient); // Compile error if case missed
	}
}
```
...
```tsx
<div className="gap-md mb-lg flex flex-col">
...
See [prisma/schema.prisma](./prisma/schema.prisma) for the current schema.
...
export function MyForm() {
	...
	return (
		<form action={action} className="gap-md flex flex-col">
			<Field>
...
switch (type) {
	...
	case "B":
		return handleB();
	default:
		return assertUnreachable(type); // Type error if case C added
}
```
